##############################################################################################
# **************      LC-OverTakeRight: Double-bit-flip - Semi-permanent        **************
##############################################################################################
import os, sys
import traci
...
def SUFI_golden_run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if time_step == 100:  # 100 refers to time 10s in the simulation
            default_value_LC_OverTakeRight = traci.vehicle.getParameter('5', "laneChangeModel.lcOvertakeRight")
        time_step += 1
    traci.close()
    return default_value_LC_OverTakeRight
##############################################################################################
...
def SUFI_fault_injection_run(Initation_time, Target_vehicle, Target_parameter, Target_value):
    sumoCmd = ["sumo", "-c", "SumoRun.config.sumocfg",
             "--tripinfo-output","output/--Nr ={: }  t ={:.2f} tripinfo.xml".format(Ex_Nr, Injection_time),
             "--fcd-output", "output/--Nr ={: }  t ={:.2f} fcd.xml".format(Ex_Nr, Injection_time),
             "--netstate-dump", "output/--Nr ={: }  t ={:.2f} Dump.xml".format(Ex_Nr, Injection_time),
             "--full-output", "output/--Nr ={: }  t ={:.2f} Full.xml".format(Ex_Nr, Injection_time),
             "--amitran-output", "output/--Nr ={: }  t ={:.2f} Trajectory.xml".format(Ex_Nr, Injection_time),
             "--lanechange-output", "output/--Nr ={: }  t ={:.2f} lanechange.xml".format(Ex_Nr, Injection_time),
             "--error-log", "output/--Nr ={: }  t ={:.2f} Error.xml".format(Ex_Nr, Injection_time)
               ]
    print("SUFI Experiment Run is in Progress ...\n\n")
    traci.start(sumoCmd)
    time_step = 0
    ...
    df = pd.DataFrame(
        {
            ...
            'Flipped bits': LIST_Flipped_bits
        }
    )
    # Current Time
    now = datetime.now()
...
for t in numpy.arange(11.0, 21.0, 0.5): # Loop for fault injection TIME interval
    ...
    randomList = [] # List to log the selected random numbers to prevent the repetitiveness
    for i in range(0, 100, 1): # Loop to define how many times to do bit-flip
        ...
        #======= Functions to convert float to binary and reverse and random selector
        def bin2float(b):
        ...
        def random_selector():
            ...
            return numbers
        # ====================  Random number selection for flipping two bits
        random_numbers = random_selector()
        if random_numbers not in randomList:
            randomList.append(random_numbers)
            randomList.append(random_numbers[::-1])
        elif random_numbers in randomList:
            while random_numbers in randomList:
                random_numbers = random_selector()
            randomList.append(random_numbers)
            randomList.append(random_numbers[::-1])
        LIST_Flipped_bits.append(random_numbers)
        # ====================  Bit flipping
        Default_value= float(Default_value)  # converts string default value to float
        Binary_value = float2bin(Default_value)    # converts float to Binary format
        ...
        for h in random_numbers:
            if int(Binary_value_list[h]) == 1:
                Binary_value_list[h] = '0'
            else:
                Binary_value_list[h] = '1'
        ...
        Target_vehicle = '5'
        Target_parameter = "laneChangeModel.lcOvertakeRight"

#==========================================Single Bit-Flip - Transient - lcOvertakeRight==================================
import os
import sys
import optparse
import numpy
...
def run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if step == 100:
            default = traci.vehicle.getParameter('5', "laneChangeModel.lcOvertakeRight")
...
for j in numpy.arange(11.0, 21.00, 0.5): # Loop for fault injection TIME interval =======================================
    ...
    for i in range(0, 64, 1): # Loop to define how many times to do bit-flip ===========================================
        ...
        print("value= ", value)
        if value == math.inf:  # If the selected value be infinity, it considers it as 99e+1000 since sumo does not accept infinity
            infinityNum+=1
        ...
        def run():
            ...
            while traci.simulation.getMinExpectedNumber() > 0:
                ...
                if step == (round(j, 3) * 10) + 1:
                     traci.vehicle.setParameter('5', "laneChangeModel.lcOvertakeRight", default_value[0])
                if step == (round(j, 3) * 10) + 3:
                    xc = traci.vehicle.getParameter('5', "laneChangeModel.lcOvertakeRight")
...
df = pd.DataFrame(
        {
                ...
                'time': jj,
                'Number': Number,
                'Bit-flip-value (lcOvertakeRight)': valueL,

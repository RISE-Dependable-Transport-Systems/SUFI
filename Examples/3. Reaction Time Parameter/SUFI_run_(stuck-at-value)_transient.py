##############################################################################################
# **************         Reaction-Time: Stuck-at-value - Transient              **************
##############################################################################################
import os, sys
import traci
...
def SUFI_golden_run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if time_step == 100:  # 100 refers to time 10s in the simulation
            default_value_Reaction_time = traci.vehicle.getParameter('5', 'device.driverstate.maximalReactionTime')
        time_step += 1
    traci.close()
    return default_value_Reaction_time
##############################################################################################
...
def SUFI_fault_injection_run(Initation_time, Target_vehicle, Target_parameter, Target_value):
    sumoCmd = ["sumo-gui", "-c", "SumoRun.config.sumocfg",
...
for t in numpy.arange(11.0, 21.0, 0.5): # Loop for fault injection TIME interval
    ...
    Reaction_time = 0.1 # Reaction Time
    for i in range(50): # Loop to define how many times to select a random value for each Initiation_time
        ...
        Reaction_time += 0.01 # Values for the OverTakeRight parameter
        value = round(Reaction_time, 3)
        ...
        Target_vehicle = '5'
        Target_parameter = "device.driverstate.maximalReactionTime"
